...
```
```{r}
pasCts<-read.csv("～/donor1.csv",row.names = 1)
```
...
save width data
```{r}
write.csv(pasAnno4, file = "～/rawdata.csv", row.names = F, quote = F, na="0")
```
...
```{r}
getwd()
mycounts<-read.csv("～/rawdata.csv",row.names = 1)
...
```{r}
write.csv(res2,file="～/drawdata.csv",quote = F)
```


Data processing and import for volcano plot
```{r}
df<-read.csv("～/drawdata.csv",header=T,stringsAsFactors=F)
...
Tag UP and DOWN group
```{r}
df$group<-ifelse(df$log2FoldChange>=2&df$pvalue<=0.05,"Up",
